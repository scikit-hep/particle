...
def produce_files(particle2008, particle2021, version, year):
    ...
    with data.files.joinpath("mass_width_2008.fwf").open() as fwf_f:
        with data.files.joinpath("pdgid_to_latexname.csv").open() as csv_f:
    ...
    with data.files.joinpath("mass_width_" + year + ".mcd").open() as mcd_f:
        ext_table = get_from_pdg_mcd(mcd_f)

    with data.files.joinpath("mass_width_2008_ext.fwf").open() as fwf_f:
        with data.files.joinpath("pdgid_to_latexname.csv").open() as csv_f:
...
def convert(version, output, fwf, latex=None):
    ...
    latexes = [data.files / "pdgid_to_latexname.csv"]
    if latex:
